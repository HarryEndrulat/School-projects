...
volatile boolean negativeFlag=false;

volatile uint8_t displayArray[8] = {0, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10};
volatile uint8_t convertedDisplayArray[8] = {0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10};
volatile uint8_t negatedDisplayArray[8] = {0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10};
volatile int MSD=0;
volatile unsigned long total=0;
volatile unsigned long convertedTotal=0;
volatile long negativeTotal =0;
...
const uint8_t sevenSegments[18] = {
  0b01111110, 0b00110000, 0b01101101, 0b01111001, 0b00110011, 0b01011011, 0b01011111, 0b01110000, 0b01111111, 0b01110011, 0b01110111, 0b00011111, 0b00001101, 0b00111101, 0b01001111, 0b01000111, 0b00000000, 0b00000001
};
...
ISR(TIMER1_COMPA_vect){
  timerCounter++;
  if((timerCounter%500)==0){//about half a second
    Serial.println(timerCounter);
  }
  if(LEDFlag==true){
    uint8_t timerFlag = ioPorts[A0_A5].input & 0x10;//left switch: to the left(0) :20 seconds, to the right(1):7.5 seconds
    timerFunction(timerFlag);
  }
}
...
void startConversion(uint8_t numberBase){
  //convert to hex or dec base on flag, when =0(left) decimal mode; when 1(right) Hex mode
  //call stop conversion when LEDFlag has been reset to false
  if(numberBase){//hex mode : change to decimal
    
  }
  else{//decimal mode : change to hex
    
  }
}

void stopConversion(){
  //"converts" the display back to the original by iterating through displayArray
  //maybe change displayArrayFunction to take an array variable as argument
}

void timerFunction(uint8_t timerFlag){
  //TODO: print when 7.5 or 20 seconds have elapsed dependent upon left switch position
  if(timerFlag && ((timerCounter%3750)==0)){//7.5 seconds
    Serial.println("7.5 seconds have elapsed since the left button was double clicked. The display has reverted to the original number.");
    LEDFlag=false;
    digitalWrite(12, LOW);
  }
  else if((timerCounter%10000)==0){//20 seconds
    Serial.println("20 seconds have elapsed since the left button was double clicked. The display has reverted to the original number.");
    LEDFlag=false;
    digitalWrite(12, LOW);
  }
}

void negateFunction(uint8_t numberBase){
  if(numberBase){//Hex mode
    //take the
  }
  else{//dec mode
    if(!negativeFlag && (MSD<8)){//if not negative and less than 8 display 
      displayArray[MSD]=0x11;
      MSD++;
      //total=~total+1;
      Serial.print("negativeTotal value: ");
      Serial.println(negativeTotal);
      negativeFlag=true;
      displayArrayFunction();
    }
    else if(negativeFlag){//if negative
      displayArray[MSD-1]=0x10;
      MSD--;
      //total=~total+1;
  ...
  if((oldLeftButtonPosition==newLeftButtonPosition) && (oldRightButtonPosition==newRightButtonPosition)){
    ...
    if(oldRightButtonPosition && !newRightButtonPosition){//case 1 right button pressed
      ...
      if((lastLeftClick+DOUBLE_CLICK_TIME) > now){
        ...
        uint8_t numberBase = ioPorts[A0_A5].input & 0x20;//when =0(left) decimal mode; when 1(right) Hex mode
        uint8_t timerFlag = ioPorts[A0_A5].input & 0x10;//left switch: to the left(0) :20 seconds, to the right(1):7.5 seconds
        
        startConversion(numberBase);
        timerCounter=0;
        LEDFlag=true;//should this just be double click? no it changes at different times
