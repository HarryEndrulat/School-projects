...
unsigned long start_illumination =  ULONG_MAX - ILLUMINATION_TIME;
...
void demo_mode(uint8_t keypress, uint8_t numberbase, uint8_t clearflag) {
  if (millis() < now + ILLUMINATION_TIME) {
    ioPorts[D8_D13].output |= 0x10;
    Serial.println("inside LED if statement");
  }
  else{//else statement may be unneccessary
    ioPorts[D8_D13].output &= 0xef;
  }
  if (!numberbase) {
    Serial.println("inside decimal mode if statement");
    if (keypress > 9) {
      keypress = 0xff;
    ...
    Serial.println("inside hex mode else statement");
    displayData(1, sevenSegments[keypress]);
    now=millis();
  }
}
...
void loop() {
  ...
  if (!clearflag) {
    Serial.println("inside clearing if statement");
    displayData(1, 0x00);
  }
}
